...
async function getInventoryList() {
    try {
        ...
        const response = await $http(`/inventory?&option=search&offset=${current_page.value}&limit=${limit.value}&searchData=${search_data.value}`, {
            method: 'GET',
        })
...
async function getInventoryByID(id) {
    try {
        const response = await $http(`/inventory/${id}`, {
            method: "GET",
        });
...
const isEditing = computed(() => !!inventory_id.value);
...
async function createInventory() {
    try {
        ...
        inventory_list.value.unshift(response.data)
        closeModal()
    } catch (error) {
...
async function updateInventoryByID(id) {
    try {
        if (form.value.name === "" || form.value.description === "") {
            is_validation.value = true;
            return;
        ...
        const index = inventory_list.value.findIndex((item) => item.id === id);
        
        if (index !== -1) {
            inventory_list.value[index] = response.data.project;
        }
...
async function deleteInventory(id) {
    try {
        ...
        const index = inventory_list.value.findIndex(item => item.id === id);
        if (index !== -1) {
            inventory_list.value.splice(index, 1);
        }
...
async function handleApiError(error) {
    ...
    if (errors) {
        if (errors.name) {
            ...
            is_description_exist.value = errors.description[0];
        }
    } else {
...
<template>
    <div>
        <h1 class="text-2xl font-bold pb-3">Inventory Creation</h1>
        <div class="flex flex-wrap gap-3 items-center justify-between pt-3 pb-5">
            <div class="bg-white xs:w-full sm:w-full md:w-2/5 lg:w-2/5 sm-w-100">
                ...
                <div class="relative">
                    ...
                    <input v-model="search_data" v-on:keyup.enter="getInventoryList"
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg mb-4">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    ...
                    <tr v-else-if="!is_loading && inventory_list.length === 0"
                        class="bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 h-80">
                        <td colspan="5">
                            <div class="flex justify-center">
                    ...
                    <tr v-else v-for="(item, index) in inventory_list" :key="index"
                        ...
                        <td class="px-4 py-4 text-center text-gray-900">
                            ...
                            <button class="mr-2 table-btn" @click="deleteInventory(item.id)">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Pagination -->
        <div v-if="!is_loading && inventory_list.length > 0" class="flex justify-center">
            ...
            <fwb-modal v-if="is_show_modal" @close="closeModal" persistent size="5xl">
                ...
                <template #body>
                    <div class="flex flex-wrap pr-6 pl-3">
                        <div class="flex items-start pt-4 w-full">
                            <p class="w-56 pl-3 pr-3">Inventory Name</p>
                        ...
                        <div class="flex items-start pt-4 w-full">
                            <div class="flex flex-col items-start w-full">
                                <p class="w-56 pl-3 pr-3">Inventory Description</p>
                                <input type="text" v-model="form.description"
                                    :class="{ 'border-red-500': is_validation && form.description === '' }"
                                    class="block w-full p-3 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    placeholder="Enter description">
                                <span v-if="is_validation && form.description === ''" class="text-red-500">
                                    Please enter inventory description
                                </span>
                ...
                <template #footer>
                    <div class="flex justify-end gap-3">
                        ...
                        <fwb-button v-else @click="isEditing ? updateInventoryByID(inventory_id) : createInventory()" color="green">
