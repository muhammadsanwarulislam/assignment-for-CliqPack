...
async function getItemList() {
  try {
    ...
    const response = await $http(
      `/item?option=search&offset=${current_page.value}&limit=${limit.value}&searchData=${search_data.value}`,
      {
        method: "GET",
      }
    );
...
async function getItemByID(id) {
  try {
    const response = await $http(`/item/${id}`, {
      method: "GET",
    });
...
async function getInventoryList() {
  try {
    const response = await $http(`/inventory`, {
      method: "GET",
    });
    inventory_list.value = response.data;
  } catch (error) {
    console.log(error);
  }
}
...
const form = ref({name: "", description: "", inventory_id: ""});
...
async function createItem() {
  try {
    ...
    spinner.value = true;
    const response = await $http(`/item`, {
      method: "POST",
      body: form.value,
    });
    push.success(response.message);
    item_list.value.unshift(response.data);
    closeModal();
  } catch (error) {
...
async function updateItemByID(id) {
  try {
    ...
    const index = item_list.value.findIndex((item) => item.id === id);

    if (index !== -1) {
      item_list.value[index] = response.data.item;
    }
...
<template>
  <div>
    <h1 class="text-2xl font-bold pb-3">Item Creations</h1>
    <div class="flex flex-wrap gap-3 items-center justify-between pt-3 pb-5">
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <button class="button btn" @click="showModal">+ Add Item</button>
      </div>
    </div>
    <!-- Table -->
      ...
      >
        ...
        >
          <tr class="bg-white">
            ...
            <th scope="col" class="px-4 py-4 text-left">Description</th>
            <th scope="col" class="px-4 py-4 text-center">Inventory</th>
        ...
        <tbody>
          ...
          <tr
            v-else-if="!is_loading && item_list.length === 0"
            class="bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 h-80"
          >
            <td colspan="5">
              <div class="flex justify-center">
          ...
          >
            ...
            <td class="px-4 py-4 text-center text-gray-900">
              ...
              <button class="mr-2 table-btn" @click="deleteItem(item.id)">
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
    ...
    <!-- Pagination -->
    <div v-if="!is_loading && item_list.length > 0" class="flex justify-center">
      <fwb-pagination
      ...
      <fwb-modal v-if="is_show_modal" @close="closeModal" persistent size="5xl">
        ...
        <template #body>
          <div class="flex flex-wrap pr-6 pl-3">
            ...
            <div class="flex items-start pt-4 w-full sm:w-1/2 lg:w-1/2">
              <p class="w-56 pl-3 pr-3">Description</p>
              <div class="flex flex-col items-start w-full">
                <input
                  type="text"
                  v-model="form.description"
                  :class="{
                    'border-red-500': is_validation && form.description === '',
                  }"
                  class="block w-full p-3 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                  placeholder="Enter Description"
                />
                <span
                  v-if="is_validation && form.description === ''"
                  class="text-red-500"
                  >Please enter description
                </span>
                <span v-if="is_description_exist" class="text-red-500">
                  {{ is_description_exist }}
                </span>
              </div>
            </div>
            <div class="flex items-start pt-4 w-full sm:w-1/2 lg:w-1/2">
              <label
                for="activity"
                class="w-56 block pl-3 pr-3 text-sm font-medium text-gray-900 dark:text-white"
                >Inventory</label
              >
              <div class="flex flex-col items-start w-full">
                <select
                ...
                <span
                  v-if="is_validation && form.inventory_id === ''"
                  class="text-red-500"
        ...
        <template #footer>
          <div class="flex justify-end gap-3">
            ...
            <fwb-button
              v-else
              @click="isEditing ? updateItemByID(item_id) : createItem()"
              color="green"
            >
